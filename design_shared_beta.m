...
beta_extent = [15, 30];   % [deg] helix angle 범위
...
for m_n = m_n_lst
    for Np1 = Np_min : Np_max
        Ng1_lo = ceil( u1_extent(1) * Np1 ); % 기어비를 만족하는 기어의 최소잇수
        Ng1_hi = floor(u1_extent(2) * Np1 ); % 기어비를 만족하는 기어의 최대잇수
        for Ng1 = Ng1_lo : Ng1_hi
            u1    = Ng1 / Np1;  % 최종 기어비 (1단)     
            Sigma = Np1 + Ng1;  % 공유 잇수 합 (중심거리 : a = m_n*Sigma/(2*cosd(beta))인데, 현재는 1단과 2단에서 같은 모듈과 비틀림각을 사용)

            % 2단: 같은 Sigma 에서 u2 범위를 만족하는 Np2 후보
            Np2_lo = ceil( Sigma / (1 + u2_extent(2)) );    % Np2(1+u2) = Np2 + Ng2를 만족시키는 2단 피니언의 최소잇수
            Np2_hi = floor(Sigma / (1 + u2_extent(1)) );    % Np2(1+u2) = Np2 + Ng2를 만족시키는 2단 피니언의 최대잇수
            for Np2 = Np2_lo : Np2_hi
                Ng2 = Sigma - Np2;
                if Ng2 <= 0, continue; end  % 기어가 더 큰 경우 제외
                u2 = Ng2 / Np2;             % 최종 기어비 (2단)
                if u2 < u2_extent(1) || u2 > u2_extent(2), continue; end    % 최종 기어비가 허용범위 벗어나면 제외
                ...
                % -------- 내부 최적화: x = [beta, db_ratio1, db_ratio2] (**최적화할 연속변수 3개: beta, dp1/b1, dp2/b2) --------
                x0 = [mean(beta_extent), mean(db_extent), mean(db_extent)]; % 초기값
                lb = [beta_extent(1),    db_extent(1),    db_extent(1)];    % 하한
                ub = [beta_extent(2),    db_extent(2),    db_extent(2)];    % 상한

                % -----목적함수 ----- (블랭크 부피 최소화)
                objf = @(x) volume_obj(x, m_n, Np1, Ng1, Np2, Ng2);
                % fmincon의 목적함수는 x(beta, db_ratio1, db_ratio2)만을 인자로 받음. (나머지는 그 시점에서 상수값)
                % -> 인자와 그 당시 상수값 바탕으로 계산되는 volume_obj가 목적함수.

                % -----제약조건 ----- (안전계수 (AGMA 강도평가), 접촉비, 언더컷 제약)
                conf = @(x) constr(x, m_n, Np1, Ng1, Np2, Ng2, ...
                                   alpha_n, n_in, T_in, N_cyc, ...
                                   SF_F_min, SF_H_min, eps_alpha_min);
                                   % 마찬가지로 x 를 제외한 값은 상수.
                % constr는 [c, ceq]를 반환. fmincon은 c(x) <= 0, ceq(x) == 0 제약을 만족하는 해를 찾음 (현재는 모두 부등식 조건이므로 ceq = [])

                [xo, fo, flag] = fmincon(objf, x0, [],[],[],[], lb, ub, conf, opts);
                % xo: (beta, db_ratio1, db_ratio2)에 대한 최적해
                % fo: 최적 목적함수 값 (최소 블랭크 부피)
                % flag: 최적화 종료 상태 플래그 (1: 수렴, 0 또는 음수: 실패)

                if flag <= 0, continue; end          % 수렴 실패

                cc = conf(xo);                      % 미세 위반 재확인 (일반적으로 사용되는 검산)
                if any(cc > 1e-4), continue; end    % 제약조건 위반 (허용 오차 1e-4 이상)

                % 결과 저장 (보고용 재계산 = eval_stage 재사용. fmincon은 내부 중간 계산 버리고 xo만 반환)
