...
% 임시값
% beta1 = 30, beta2 = 25 (언더컷 free 잇수가 가장 작게 나옴)
...
% 가정 : 두 단 모듈 통일, 헬릭스각은 단별로 독립 최적화 (β2는 중심거리 등식에서 결정)
...
% Sigma2/Sigma1 비율의 허용 범위:
%   a 등식 m_n*Sigma1/cosβ1 = m_n*Sigma2/cosβ2, β1·β2 ∈ [beta_min,beta_max] 에서 실수해가 존재하려면,
ratio_lo = cosd(beta_extent(2)) / cosd(beta_extent(1)); % 하한 (β1=max → β2=min)
ratio_hi = cosd(beta_extent(1)) / cosd(beta_extent(2)); % 상한 (β1=min → β2=max)
...
for m_n = m_n_lst
    for Np1 = Np1_min : Np_max
        ...
        for Ng1 = Ng1_lo : Ng1_hi
            ...
            if Np1 < gear.interference_free_p(alpha_n, beta_max, u1), continue; end % 1단 언더컷 사전필터
            ...
            for Np2 = Np2_min : Np_max
                ...
                for Ng2 = Ng2_lo : Ng2_hi
                    ...
                    if Np2 < gear.interference_free_p(alpha_n, beta_max, u2), continue; end % 2단 언더컷 사전필터

                    % 사전필터: Sigma2/Sigma1 비율이 허용 범위를 벗어나면 어떤 beta1로도 beta2 실수해 없음 -> skip
                    ratio = Sigma2 / Sigma1;
                    ...
                    % beta2 = acosd( (Sigma2/Sigma1)*cosd(beta1) ) 로 유도 (중심거리 등식)
                    ...
                    % beta2 유도 (보고용)
                    beta2_opt = acosd( (Sigma2/Sigma1) * cosd(xo(1)) );

                    % 결과 저장 (보고용 재계산 = eval_stage 재사용)
                    s1 = eval_stage(m_n, xo(1),    xo(2), Np1, Ng1, alpha_n, n_in, T_in, N_cyc);
                    s2 = eval_stage(m_n, beta2_opt, xo(3), Np2, Ng2, alpha_n, n_in, T_in, N_cyc);
